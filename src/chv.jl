...
function solve(problem::PDMPProblem, algo::CHV{Tode}; verbose::Bool = false, ind_save_d=-1:1, ind_save_c=-1:1, dt=0.001, n_jumps = Inf64) where {Tode <: Symbol}
	...
	if ode == :cvode
		Flow = (X0_,Xd_,Δt,r_)->Sundials.cvode(  (tt,x,xdot)->f_CHV!(problem.caract.F,problem.caract.R,tt,x,xdot,Xd_,problem.caract.parms,r_), X0_, [0., Δt], abstol = 1e-9, reltol = 1e-7, integrator = :BDF)
	elseif	ode==:bdf
		Flow = (X0_,Xd_,Δt,r_)->Sundials.cvode(  (tt,x,xdot)->f_CHV!(problem.caract.F,problem.caract.R,tt,x,xdot,Xd_,problem.caract.parms,r_), X0_, [0., Δt], abstol = 1e-9, reltol = 1e-7, integrator = :BDF)
	elseif	ode==:adams
		Flow = (X0_,Xd_,Δt,r_)->Sundials.cvode(  (tt,x,xdot)->f_CHV!(problem.caract.F,problem.caract.R,tt,x,xdot,Xd_,problem.caract.parms,r_), X0_, [0., Δt], abstol = 1e-9, reltol = 1e-7, integrator = :Adams)
	elseif ode==:lsoda
		Flow = (X0_,Xd_,Δt,r_)->LSODA.lsoda((tt,x,xdot,data)->f_CHV!(problem.caract.F,problem.caract.R,tt,x,xdot,Xd_,problem.caract.parms,r_), X0_, [0., Δt], abstol = 1e-9, reltol = 1e-7)
	elseif ode==:euler
