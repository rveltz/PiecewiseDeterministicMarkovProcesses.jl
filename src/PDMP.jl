...
function pdmp!{T}(xc0::Vector{Float64},xd0::Array{Int64,1},F::Base.Callable,R::Base.Callable,DX::Base.Callable,nu::Matrix{Int64},parms::Vector{T},ti::Float64, tf::Float64,verbose::Bool = false;ode=:cvode,algo=:chv,dt = 0.1, n_jumps = 1000)
	...
	# rate = zeros(size(nu,1))
	# R(rate,xc0,xd0,0.,parms,false)
	# R_wrap = R
	#
	# if 1==1 # we have a rate function suited to the rejection algorithm, one could also have tested typeof(res[1]) == Vector
	# 	if algo==:rejection
	...
	elseif algo==:chv_optim
		return PDMP.chv_optim!(n_jumps,xc0,xd0,F,R,DX,nu,parms,ti, tf,verbose,ode=ode)
	elseif algo==:rejection
		PDMP.rejection(n_jumps,xc0,xd0,F,R_wrap,DX,nu,parms,ti, tf,verbose,ode=ode)
		# elseif algo==:rejection_exact
		# 			return PDMP.rejection_exact(n_jumps,xc0,xd0,F,R,DX,nu,parms,ti, tf,verbose,ode=ode)
	elseif contains(string(algo),"tauleap")
		PDMP.tauleap(n_jumps,xc0,xd0,F,R_wrap,nu,parms,ti, tf,verbose,ode=ode,dt=dt,algo=algo)
	end
end
